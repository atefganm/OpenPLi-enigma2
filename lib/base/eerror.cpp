...
void eDebugImpl(int flags, const char* fmt, ...)
{
	...
	if (debugTime && !(flags & _DBGFLG_NOTIME)) {
		...
		pos = snprintf(buf, eDEBUG_BUFLEN, "<%6llu.%03lu> ", (long long)tp.tv_sec, tp.tv_nsec/1000000);
	}
	...
	if (vsize < 0) {
		...
		if (debugTime && !(flags & _DBGFLG_NOTIME))
			pos = snprintf(buf, pos + vsize, "<%6llu.%03lu> ", (long long)tp.tv_sec, tp.tv_nsec/1000000);
		va_start(ap, fmt);
